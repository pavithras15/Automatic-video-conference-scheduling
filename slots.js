...
router.post('/respond', async (req, res) => {
  try {
    ...
    if (confirmedSlot && !(await eventAlreadyScheduled())) {
      ...
      try {
        ...
        for (const email of participants) {
          await sendMail({
            ...
            text: `Your interview is scheduled for ${start.toLocaleString()}. Meet link: ${event.hangoutLink}`,
            html: `<p>Your interview is scheduled for <b>${start.toLocaleString()}</b>.<br>Meet link: <a href=\"${event.hangoutLink}\">${event.hangoutLink}</a></p>`
          });
        }
